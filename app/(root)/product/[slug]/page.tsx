      ...
      <div className="grid gap-8 lg:grid-cols-2 lg:items-start">
        ...
        <div className="flex flex-col space-y-6 rounded-2xl border border-gray-200 bg-white p-6 lg:p-8">
          ...
          <div className="space-y-3">
            <h1 className="text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl">{product.name}</h1>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-3">
              {["S", "M", "L", "XL", "XXL"].map((size, index) => (
                <Button key={size} variant={index === 0 ? "default" : "outline"} className="h-12 min-w-16 rounded-full">
                  {size}
                </Button>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-gray-200 p-5">
            <h2 className="mb-3 text-lg font-semibold text-gray-900">Description & Fit</h2>
            <p className="text-sm leading-6 text-gray-600">{product.description || "No description available."}</p>
          </div>
        </div>
      </div>
    </section>
